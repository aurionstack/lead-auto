...
export function validateCampaignTarget(category: string, location: string): string | null {
  if (!isUnitedStatesLocation(location)) {
    return 'This pilot is limited to United States locations.';
  }
  if (!isHomeServiceCategory(category)) {
    return 'This pilot is limited to HVAC, plumbing, roofing, and garage-door companies.';
  }
  return null;
}

...
export default function DashboardClient({ initialLeads, isMockData = false }: DashboardClientProps) {
  ...
  useEffect(() => {
    ...
    if (first) setEditedPitch(first.drafted_pitch ?? '');
  }, [initialLeads]);
  ...
  const updateLead = useCallback(
    async (id: string, payload: { status?: LeadStatus; drafted_pitch?: string }) => {
  ...
  const handleSavePitch = async () => {
    if (!selectedLead || !pitchModified) return;
    setIsUpdating(true);
    try {
      await updateLead(selectedLead.id, { drafted_pitch: editedPitch });
      setLeads((prev) =>
        prev.map((l) =>
          l.id === selectedLead.id ? { ...l, drafted_pitch: editedPitch } : l
        )
      );
      setPitchModified(false);
      showFeedback('success', 'Pitch saved successfully.');
    } catch (err) {
            ...
            <div className="max-w-3xl mx-auto p-6 space-y-6">
              ...
              {selectedLead.ai_reasoning ? (
                <div className="rounded-xl bg-indigo-950/30 border border-indigo-900/40 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs text-indigo-600 bg-indigo-900/30 px-2 py-0.5 rounded-full">Gemini 2.5 Flash</span>
                  </div>
              ...
              <div className="rounded-xl bg-slate-900/60 border border-slate-800/60 p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-white">WhatsApp Pitch</h3>
                    {pitchModified && (
                ...
                <p className="text-xs text-slate-600 mt-2">
                  Edit the message before sending. Changes are saved before opening WhatsApp.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 pb-6">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-3 px-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 disabled:cursor-not-allowed disabled:text-slate-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-900/30"
                >
                  {isUpdating ? (
                  ...
                  {selectedLead.status === 'contacted' ? 'Already Contacted' : 'Open WhatsApp Web'}
                </button>
